...
if menu == "📹 ระบบสแกนใบหน้าหน้างาน":
    ...
    # --- ระบบโหลดโมเดลใบหน้าสำหรับ Cloud ---
    ...
    if 'local_check_in_cache' not in st.session_state:
        st.session_state.local_check_in_cache = {}
    ...
    with col1:
        ...
        if not st.session_state.logged_in:
            ...
            with st.form("student_registration_form", clear_on_submit=True):
                ...
                file1 = st.file_uploader("รูปที่ 1", type=["jpg", "jpeg", "png"], key="f1")
                file2 = st.file_uploader("รูปที่ 2", type=["jpg", "jpeg", "png"], key="f2")
                file3 = st.file_uploader("รูปที่ 3", type=["jpg", "jpeg", "png"], key="f3")
                file4 = st.file_uploader("รูปที่ 4", type=["jpg", "jpeg", "png"], key="f4")
                file5 = st.file_uploader("รูปที่ 5", type=["jpg", "jpeg", "png"], key="f5")
            ...
            if submit_btn:
                if not full_name.strip():
                    ...
                    st.warning("⚠️ กรุณาอัปโหลดรูปภาพให้ครบ")
                else:
                    saved_paths = []
                    ...
                    try:
                        ...
                        st.error(f"บันทึกรายชื่อขัดข้อง: {register_err}")
                ...
                for (x, y, w, h) in faces:
                    ...
                    if not already_exists:
                        ...
                        try:
                            ...
                            st.write(f"⚠️ บันทึกข้อมูลลงชีตขัดข้อง: {sheet_err}")
...
# =========================================================================
    ...
    if df_log is not None and not df_log.empty:
        ...
        with m1:
            st.metric(label="📈 จำนวนครั้งสแกนรวมในช่วงนี้", value=f"{len(df_filtered)} ครั้ง")
        with m2:
            st.metric(label="🧑‍🎓 จำนวนนักเรียนที่มาสแกน (ไม่นับชื่อซ้ำ)", value=f"{df_filtered['ชื่อ - นามสกุล'].nunique()} คน")
        with m3:
            st.metric(label="📅 ช่วงเวลาที่กำลังแสดง", value=f"{start_date.strftime('%d/%m/%Y')} ถึง {end_date.strftime('%d/%m/%Y')}")
